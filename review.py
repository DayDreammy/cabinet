...
def build_keyword_prompt(query: str, max_keywords: int) -> str:
    return (
        ...
        f"- 正常返回 3-7 个关键词，最多 {max_keywords} 个。\n"
        "- 优先具体概念，避免空泛词；关键词以中文为主。\n"
        "- 优先从下方“候选关键词库”中选取最相关项；必要时可补充少量新词。\n"
        "- 关键词列表中必须包含 1-2 个来自问题原句的核心关键词（保持原词不改写）。\n"
        "- 只输出 JSON。\n\n"
        "候选关键词库：\n"
        ...
        "示例：\n"
        "问题：恋人在争吵后的冷静期，适合做些什么？\n"
        "只输出 JSON：{\"keywords\": [\"爱\", \"冷静期\", \"情绪\", \"沟通\", \"亲密关系\", \"冲突\", \"恋人\", \"争吵\"]}\n\n"
        "问题：从大厂转去创业公司，应该评估哪些关键风险？\n"
        "只输出 JSON：{\"keywords\": [\"大厂\", \"创业\", \"职业\", \"事业\", \"风险\", \"反脆弱\"]}\n\n"
        "问题：如何在亲子关系中建立边界，又不伤害孩子的安全感？\n"
        "只输出 JSON：{\"keywords\": [\"亲子关系\", \"边界\", \"安全感\", \"父母\", \"子女\", \"沟通\"]}\n\n"
        "问题：关系里信任被破坏后，修复的关键步骤是什么？\n"
        "只输出 JSON：{\"keywords\": [\"信任\", \"亲密关系\", \"个人成长\", \"贵人\", \"沟通\", \"责任\", \"原谅\"]}\n\n"
        "问题：学习一门新技能时，如何建立有效的反馈闭环？\n"
        "只输出 JSON：{\"keywords\": [\"学习\", \"新技能\", \"反馈\", \"闭环\", \"方法论\", \"元技能\", \"自我认知\", \"自我成长\"]}\n\n"
        f"问题：{query}\n\n"
        '只输出 JSON：{"keywords": ["...", "..."]}'
    )
